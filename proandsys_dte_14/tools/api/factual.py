...
GET_DOCUMENTS_XML = 'document/receive/'
CONFIRM_DOCUMENTS_XML = 'document/confirm/'
GET_TRACK_ID = 'document/dtestatus/'
...
def set_caf(url, data, user, passwd):
    r = requests.post(url + CAF_URL, data=json.dumps(data), auth=HTTPBasicAuth(user, passwd), headers=HEADERS)
...
def send_transfer_dte(url, data, user, passwd):
    r = requests.post(url + TRANSFER_URL, data=json.dumps(data), auth=HTTPBasicAuth(user, passwd), headers=HEADERS)
    return json.loads(r.text)
...
def get_documents_xml_info(url, data, user, passwd):
    r = requests.get(url + GET_DOCUMENTS_XML + str(data), auth=HTTPBasicAuth(user, passwd), headers=HEADERS)
    return json.loads(r.text)


def confirm_documents_xml(url, data, user, passwd):
    HEADERS.update({'Direct': 'true'})  # Quitar en caso de desear validacion por lote al final del dia.
    r = requests.post(url + CONFIRM_DOCUMENTS_XML, auth=HTTPBasicAuth(user, passwd), headers=HEADERS)
    return json.loads(r.text)


def get_track_id(url, sender, dte_type, number, user, passwd):
    r = requests.get(url + GET_TRACK_ID + sender + '/' + dte_type + '/'+ number + '/received', auth=HTTPBasicAuth(user, passwd), headers=HEADERS)
    return json.loads(r.text).get('TrackId')
